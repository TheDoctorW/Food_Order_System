...
class OrderingBunsForm():
    
    def __init__(self, form):
        self._bunsnumber = Field('buns_number', [InputRequired('Buns number cannot be empty!')])
    ...
    def _parse(self, form):
        ...
        if self._bunsnumber.is_valid:
            if (self._bunsnumber.data).isdigit() == False:
                self._other_errors['buns_number_type'] = 'Please enter a valid number!'
        if self._bunsnumber.is_valid and (self._bunsnumber.data).isdigit() == True:
            if (int(self._bunsnumber.data) > 3):
                self._other_errors['buns_limit'] = 'Please enter a reasonable quantity!'
